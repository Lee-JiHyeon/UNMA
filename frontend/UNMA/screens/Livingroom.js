...
class Livingroom extends Component {
  ...
  componentDidMount() {
    ...
    AsyncStorage.getItem('childPk', (err, res) => {
      ...
      .then((res) => {
        console.log(res)
        this.setState({ livingroomitems: res.data })
      })
      .catch((err) => {
        console.error(err)
      })
  ...
  render() {
    const { livingroomitems } = this.state;
    const { language } = this.state;
    ...
    return (
      <Block>
        ...
        <Block style={styles.container}>
          ...
          <Image 
          resizeMode='stretch'
          source={{uri : 'https://image.freepik.com/free-vector/diving-room-with-sofa-tv-decoration_24640-45098.jpg'}} style={styles.background}></Image>
        </Block>
      </Block>
    );
  }
