    ...
    return (
      <Block>
        <Block flex={false} row center space="between" style={styles.header}>
          ...
          >
            <Block flex={false} row space="between" style={styles}>
              {roads.map((road) => (
                <TouchableOpacity
                  key={road.character_eng_name}
                  onPress={() => {this.getData(road[language], road.checked, road.open_pic_dir, road.close_pic_dir)}}
                >
                 <Badge
                    center
                    middle
                    margin={[0, 10, 15]}
                    size={60}
                    color="rgba(0,150,151,0.08)"
                  >
                    <Image source={road.checked ? { uri : road.open_pic_dir } : { uri : road.close_pic_dir }} style={styles.stories} />
                  </Badge>
                </TouchableOpacity>
              ))}
            </Block>
        ...
        <Block style={styles.container}>
          {/* <LottieView
          source={require("../animations/44142-bus-animation.json")}
          autoPlay
          style={styles.car}
        /> */}
          ...
          <Image 
          resizeMode='stretch'
          source={{uri : 'https://image.freepik.com/free-vector/cityscape-scene-with-crosswalk-road_24640-46230.jpg'}} style={styles.background}></Image>
        </Block>
      </Block>
    );
  }
