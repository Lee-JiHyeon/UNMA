    ...
    return (
      <Block>
        <Block flex={false} row center space="between" style={styles.header}>
          ...
          >
            <Block flex={false} row space="between">
              {animals.map((animal) => (
                <TouchableOpacity
                  key={animal.character_eng_name}
                  onPress={() => {this.getData(animal[language], animal.checked, animal.open_pic_dir, animal.close_pic_dir)}}
                >
                  <Badge
                    center
                    middle
                    margin={[0, 10, 15]}
                    size={60}
                    color="rgba(0,150,151,0.10)"
                  >
                    <Image source={animal.checked ? {uri : animal.open_pic_dir } : {uri : animal.close_pic_dir}} style={styles.stories} />
                  </Badge>
                </TouchableOpacity>
              ))}
            </Block>
        ...
        <Block style={styles.container}>
          {/* <LottieView
          source={require("../animations/6686-sun.json")}
          autoPlay
          style={styles.sun}
        /> */}
          ...
          <Image source={{uri : 'https://image.freepik.com/free-vector/forest-landscape-with-pine_18591-51152.jpg'}} style={styles.background}></Image>
        </Block>
      </Block>
    );
  }
